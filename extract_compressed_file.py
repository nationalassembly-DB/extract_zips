...
def extract_bandizip(compress_file_path):
    ...
    try:
        ...
        zips_extract_folder_alt = os.path.join('\\\\?\\', os.path.dirname(
            compress_file_path), '[alt] ' + os.path.splitext(os.path.basename(compress_file_path))[0])
        ...
        if not os.path.exists(zips_extract_folder):
            ...
            os.makedirs(zips_extract_folder_alt)
            subprocess.run(
                ["C:\\Program Files\\Bandizip\\Bandizip.exe", "bx", "-y",
                 f"-o:{zips_extract_folder_alt}", compress_file_path],
                check=True
            )
            e_s = "동일 폴더명 문제 발생"
